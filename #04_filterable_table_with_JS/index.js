...
for (let i = 0; i < dataKeys.length; i++) {
  ...
  th.insertAdjacentHTML('beforeend', '<i class="fas fa-sort sort-btn thead-btn"></i>');
  th.insertAdjacentHTML('beforeend', '<i class="fas fa-filter filter-btn thead-btn"></i>');
  tr.appendChild(th);
}
...
function resetSortClass() {
  for (let i = 0; i < sortBtns.length; i++) {
    sortBtns[i].className = "fas fa-sort sort-btn thead-btn";
  }
}
function resetFilterClass() {
  for (let i = 0; i < filterBtns.length; i++) {
    filterBtns[i].className = "fas fa-filter filter-btn thead-btn";
  }
}
...
function drawFilter() {
  for (let i = 0; i < filterBtns.length; i++) {
    filterBtns[i].setAttribute('data-type', dataKeys[i])
    const ths = document.querySelectorAll('th');
    const filterBox = document.createElement('div');
    filterBox.classList = 'filter-box';
    const ul = document.createElement('ul');
    for (let j = 0; j < displayedData.length; j++) {
      const dataValues = Object.values(displayedData[j]);
      const li = document.createElement('li');
      const input = document.createElement('input');
      const label = document.createElement('label');
      input.setAttribute('type', 'checkbox');
      input.setAttribute('id', `${dataValues[i]}`);
      input.setAttribute('class', `${dataKeys[i]}-checkbox`);
      input.setAttribute('checked', 'true');
      input.addEventListener('click', () => {
        const page = currentPage;
        console.log(input.checked)
        if (input.checked === false) {
          displayedData.splice(j,1);          
          updateTable(page)
        }else if(input.checked === true){
          displayedData.splice(j,0,data[j]); //TODO not sure is it ok to use splice() method          
          updateTable(page)
        }
      });
      label.innerText = dataValues[i];
      li.appendChild(input);
      li.appendChild(label)
      // li.innerHTML = `<input type="checkbox" id="${dataValues[i]}" class="${dataKeys[i]}-checkbox" checked><label for="${dataValues[i]}">${dataValues[i]}</label>`;
      ul.appendChild(li);
    }
    filterBox.appendChild(ul);
    ths[i].appendChild(filterBox);
  }
}
...
function addEventListeners() {
  ...
  for (let i = 0; i < sortBtns.length; i++) {
    ...
    sortBtns[i].addEventListener('click', (e) => {
      ...
      if (sortBtnOffsetY >= 20) {
        sortBtn.className = "fas fa-sort-down sort-btn thead-btn selected";
        const sortData = data.slice().sort((a, b) => a[key] < b[key] ? -1 : 1);
        displayedData = sortData;
      } else if (sortBtnOffsetY < 20) {
        sortBtn.className = "fas fa-sort-up sort-btn thead-btn selected";
  ...
  const filterBoxes = document.getElementsByClassName('filter-box');
  for (let i = 0; i < filterBtns.length; i++) {
    filterBtns[i].addEventListener('click', (e) => {
      console.log('filterBoxes[i].style.display', filterBoxes[i].style)
      if (filterBoxes[i].style.display == '') {
        filterBoxes[i].style.display = 'block';
        filterBtns[i].classList.add('selected');
      } else if (filterBoxes[i].style.display == 'block') {
        filterBoxes[i].style.display = '';
